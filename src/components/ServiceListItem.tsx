...
export default function ServiceListItem({ service, showStatus, onEdit }: Props) {
  ...
  return (
    <a
      ...
      className="glass-card px-4 py-2.5 flex items-center gap-3 relative group cursor-pointer"
      onMouseEnter={() => setHovering(true)}
      onMouseLeave={() => setHovering(false)}
    >
      <ServiceIcon slug={service.icon} name={service.name} size={24} />
      ...
      <span className="text-xs text-gray-500 bg-surface-700 px-2 py-0.5 rounded-full ml-auto shrink-0">
        {service.category}
      </span>
